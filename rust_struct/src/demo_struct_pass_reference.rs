...
fn print_employee(e: &Employee) {
    println!("Using explicit dereferencing: {} earns {}, fulltime status: {}", (*e).name, (*e).salary, (*e).fulltime);
    println!("Using implicit dereferencing: {} earns {}, fulltime status: {}", e.name, e.salary, e.fulltime);
}
